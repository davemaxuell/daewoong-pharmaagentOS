...
export function EmployeeGuide() {
  ...
  const questions = [
    ["How does agent research work?", "에이전트 리서치는 어떻게 사용하나요?", "Open Research agent and describe the brief your team needs. You can watch the plan, searches, source passages, and evidence checks as they happen. Use Stop to pause the task, then Continue to resume saved work. A checked draft appears when the task finishes; open its citations before sharing it.", "‘리서치 에이전트’에서 팀에 필요한 브리핑을 요청하세요. 계획, 검색, 원문 확인, 근거 검토 과정을 실시간으로 볼 수 있습니다. ‘중지’하면 진행 상황을 저장하며, ‘이어서 진행’으로 다시 시작할 수 있습니다. 작업이 끝나면 검토된 초안이 표시됩니다. 공유하기 전 출처를 확인해 주세요."],
    ["Will my research continue if I close this page?", "페이지를 닫아도 리서치가 계속되나요?", "Yes. Research tasks and briefs are saved on the service and remain accessible from the same browser session. Reopen Research agent to see them. No account is required. Clearing browser cookies or using another browser removes access to this session’s tasks, so download briefs you need to keep.", "네. 리서치 작업과 브리핑은 서비스에 저장되며 같은 브라우저 세션에서 다시 열 수 있습니다. ‘리서치 에이전트’에서 확인하세요. 계정은 필요하지 않습니다. 쿠키를 삭제하거나 다른 브라우저를 사용하면 이전 세션의 작업에 접근할 수 없으니 필요한 브리핑은 다운로드해 주세요."],
  ...
  return (
    <article className={styles.guide}>
      <header>
        ...
        <h1>{text("Tell the agent what you need to investigate.", "조사할 내용을 에이전트에게 알려 주세요.")}</h1>
        ...
        <Link className="button button--primary" href="/research">{text("Start agent research", "에이전트 리서치 시작")}<ArrowRight size={18} aria-hidden="true" /></Link>
      </header>
      <section aria-labelledby="steps-heading">
        <h2 id="steps-heading">{text("For a quick question, use AI chat", "간단한 질문은 AI 대화를 이용하세요")}</h2>
      ...
      <section id="availability" className={styles.availability}>
        ...
        <p>{text("Run FDA research with visible progress and saved briefs, search letters, read original documents, ask quick AI questions, and bookmark sources. The separate My review drafts area still holds personal notes, not submitted agent tasks.", "진행 상황이 보이는 FDA 리서치와 브리핑 저장, 경고서한 검색과 원문 읽기, 빠른 AI 질문, 자료 즐겨찾기를 이용할 수 있습니다. 별도의 ‘내 검토 초안’은 개인 메모이며 에이전트에 제출한 작업이 아닙니다.")}</p>
      </section>
